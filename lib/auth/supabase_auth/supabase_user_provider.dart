...
class MontePlaySupabaseUser extends BaseAuthUser {
  MontePlaySupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> montePlaySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MontePlaySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
